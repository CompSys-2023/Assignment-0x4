            ...
            try:
                ...
                for i in range(len(network)):
                    ...
                    if address == message_from:
                        # TODO: This has been changed since it causes an exception, since tuples are immutable.
                        # Original: network[i][1] = now
                        lst = list(network[i])
                        lst[1] = now
                        network[i] = tuple(lst)

                    # Remove any network nodes we havn't heard from in a while
                    # TODO: REMOVE THIS IS USED FOR TESTING JOINING! DEFAULT VAL = 60
                    if now-timestamp > 0.1: 
                        if (address != "127.0.0.1:34567"):
